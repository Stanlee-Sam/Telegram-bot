...
const startBot = async () => {
  ...
  if (!ngrokUrl) console.warn("⚠️ Could not detect ngrok URL. /simulate may fail.");
  ...
  function requireAdmin(cmdName, handler) {
    return async (msg, match) => {
      ...
      if (!isAdmin(msg)) {
        await bot.sendMessage(msg.chat.id, `❌ Unauthorized — admin-only command: ${cmdName}`);
        return;
      }
      try {
  ...
  bot.onText(/^\/start(?:@\S+)?$/i, (msg) => {
    ...
    bot.sendMessage(msg.chat.id, "👋 Hello! Welcome to D'atrix Subscription bot! Use /help for navigation");
  });
  ...
  bot.onText(/^\/help(?:@\S+)?$/i, (msg) => {
    ...
    if (isAdminUser) {
      ...
      helpMessage += "• /broadcast Your Message - Send message to all subscribers\n";
  ...
  bot.onText(/^\/subscribe(?:@\S+)?$/i, (msg) => {
    ...
    if (pendingSubscriptions.has(fromId)) {
      bot.sendMessage(chatId, "⚠️ You already have a pending subscription request. Please send your phone number or wait.");
      return;
    }
    ...
    const timeoutId = setTimeout(() => {
      ...
      try {
        bot.sendMessage(chatId, "⏳ Subscription request timed out. Please run /subscribe again when ready.");
      } catch (e) {}
    }, PENDING_TIMEOUT_MS);

    pendingSubscriptions.set(fromId, { step: "awaiting_phone", chatId, timeoutId });
  ...
  bot.on("message", async (msg) => {
    ...
    console.log(`Pending phone input from ${fromId}: raw='${msg.text}' sanitized='${phoneNumber}'`);

    if (!/^2547\d{8}$/.test(phoneNumber)) {
      await bot.sendMessage(chatId, "❌ Invalid phone number. Use format 2547XXXXXXXX");
      return;
    }

    clearTimeout(pending.timeoutId);
    await bot.sendMessage(chatId, "✅ Phone number received! Processing your subscription...");

    const amount = 1;
    const callbackUrl = ngrokUrl ? `${ngrokUrl}/webhook/mpesa` : process.env.CALLBACK_URL;
    ...
    try {
      ...
      if (response?.ResponseCode === "0" || response?.responseCode === "0") {
        await bot.sendMessage(chatId, "✅ Payment request sent! Check your phone for the M-Pesa prompt.");
      } else {
        await bot.sendMessage(chatId, "⚠️ Payment request failed. Please try again later.");
      }
    } catch (err) {
      console.error("Subscription error:", err);
      await bot.sendMessage(chatId, "❌ Something went wrong while processing your subscription. Please try again or contact support.");
    } finally {
      pendingSubscriptions.delete(fromId);
    }
  });
  ...
  bot.onText(/^\/simulate(?:@\S+)?$/i, (msg) => {
    ...
    (async () => {
      try {
        ...
        await axios.get(`${ngrokUrl}/webhook/simulate-success/${chatId}/${amount}`);
        await bot.sendMessage(chatId, `✅ Simulated subscription created for ${chatId} amount ${amount} KES.`);
      } catch (err) {
  ...
  bot.onText(/^\/checkExpiry(?:@\S+)?$/i, requireAdmin("checkExpiry", async (msg) => {
    const count = await removeExpiredUsers(bot);
    await bot.sendMessage(msg.chat.id, `✅ Expiry check completed. Removed ${count} user(s).`);
  }));

  bot.onText(/^\/members(?:@\S+)?$/i, requireAdmin("members", async (msg) => {
    try {
      const res = await pool.query(`SELECT telegram_id, username, expiry_date FROM "Subscription" ORDER BY expiry_date DESC`);
      if (!res.rows.length) return bot.sendMessage(msg.chat.id, "No active subscribers found.");

      const list = res.rows
        .map(row => `• ${row.username ? `@${row.username}` : row.telegram_id} - ${row.telegram_id} (expires: ${row.expiry_date.toISOString().split("T")[0]})`)
        .join("\n");
  ...
  bot.onText(/^\/remove\s+(.+)(?:@\S+)?$/i, requireAdmin("remove", async (msg, match) => {
    const target = match[1].replace("@", "").trim();
    try {
      const res = await pool.query(`SELECT telegram_id FROM "Subscription" WHERE telegram_id=$1 OR username=$2`, [target, target]);
      if (!res.rows.length) return bot.sendMessage(msg.chat.id, `❌ User ${target} not found.`);
  ...
  bot.onText(/^\/broadcast\s+(.+)(?:@\S+)?$/i, requireAdmin("broadcast", async (msg, match) => {
